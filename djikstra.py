...
class Graph:
    ...
    def remove_connection(self, node1, node2):
      self.remove_connection_dir(node1, node2)
      self.remove_connection_dir(node2, node1)
   
    # Remove connection in a directed manner (node1 to node2)
    # Can accept index number OR node object
    def remove_connection_dir(self, node1, node2):
      node1, node2 = self.get_index_from_node(node1), self.get_index_from_node(node2)
...
#input for the diagram from the homework with A as the source node
a = Node("A")
b = Node("B")
c = Node("C")
d = Node("D")
